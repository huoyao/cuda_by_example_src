...
void const_raytrace()
{
  ...
  HANDLE_ERROR(cudaMalloc((void **)&dev_bitmap,SPHERES*sizeof(sphere)));
  ...
  dim3 blocks(16,16);
  kernal<<<grids,blocks>>>(const_s,dev_bitmap);
  free(temp_s);
...
void raytrace()
{
  ...
  for (int i=0;i!=SPHERES;++i)
  {
    ...
    temp_s[i].y=ran(1.0f);
    temp_s[i].z=ran(1.0f);
    temp_s[i].r=ran(1000.0f)-500.0f;
    temp_s[i].g=ran(1000.0f)-500.0f;
    temp_s[i].b=ran(1000.0f)-500.0f;
    temp_s[i].radius=ran(100.0f)+20.0f;
  }
